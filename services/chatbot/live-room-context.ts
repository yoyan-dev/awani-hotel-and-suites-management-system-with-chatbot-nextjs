...
function extractAvailabilityParams(message: string): {
  ...
  const checkInMatch = message.match(
    new RegExp(
      `(?:check\\s*-?\\s*in|from)\\s*(?:date)?\\s*[:=]?\\s*${datePattern}`,
      "i",
    ),
  );
  const checkOutMatch = message.match(
    new RegExp(
      `(?:check\\s*-?\\s*out|to|until)\\s*(?:date)?\\s*[:=]?\\s*${datePattern}`,
      "i",
    ),
  );
  ...
  const checkIn = normalizeDateToken(
    checkInMatch?.[1] ?? allDateMatches[0] ?? "",
  );
  const checkOut = normalizeDateToken(
    checkOutMatch?.[1] ?? allDateMatches[1] ?? "",
  );
...
function formatRoomTypeForPrompt(roomType: any): string {
  ...
  const addOnSummary = addOns
    .map((entry: any) => {
      ...
      const addOnPrice =
        entry?.add_on?.price != null
          ? `PHP ${Number(entry.add_on.price)}`
          : "No price";
      const remaining =
...
async function getAvailableRoomTypes(params: {
  ...
  const normalizedCheckOut =
    checkOutDate < checkInDate ? params.checkIn : params.checkOut;
...
export async function buildLiveSections(prompt: string): Promise<string[]> {
  const lowerPrompt = prompt.toLowerCase();
  const wantsRoomTypes = ROOM_TYPE_KEYWORDS.some((word) =>
    lowerPrompt.includes(word),
  );
  const wantsAvailability = AVAILABILITY_KEYWORDS.some((word) =>
    lowerPrompt.includes(word),
  );
