...
export async function GET(req: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    const limit = 10;
    const result = await listFunctionRooms({
...
export async function POST(req: Request): Promise<NextResponse<ApiResponse>> {
  try {
    const data = await createFunctionRoom(await req.formData());

    return apiSuccess(
      data,
      apiMessage("Success", "Function Room added successfully", "success"),
      201,
    );
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    const data = await deleteFunctionRooms(selectedValues);
